...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--disconnect', action='store_true', help='disconnect from the device')
        ...
        for equimpent in equipments:
            ...
            if options['connect']:
                if connection.is_alive():
                    ...
                    equimpent.save()
                    self.stdout.write(self.style.SUCCESS("Successfully connected to the device"))
                    
            if options['disconnect']:
                connection.disconnect()
                equimpent.status = "disconnected"
                equimpent.save()
                self.stdout.write(self.style.SUCCESS("Successfully disconnected from the device"))
            
                
            if not options['connect'] and not options['disconnect']:
                self.stdout.write(self.style.WARNING("Please specify --connect or --disconnect"))
